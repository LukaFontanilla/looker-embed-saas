...
const LeftNav = () => {
    ...
    return (
        <aside className="sm:min-w-1000 w-1/12 md-lg:w-2/12 border-r border-zinc-200 dark:border-zinc-800 overflow-auto" >
          ...
          <button onClick={() => setShowNav(showNav ? false : true)} data-collapse-toggle="navbar-dropdown" type="button" class="inline-flex items-center p-2 w-10 h-10 justify-center text-sm text-gray-500 rounded-lg xl:hidden hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-gray-200 dark:text-gray-400 dark:hover:bg-gray-700 dark:focus:ring-gray-600" aria-controls="navbar-dropdown" aria-expanded="false">
            ...
            {!matches && showNav && (
                ...
                {navCongfig.map((group) => <NavGroups group={group} show={showNav}/>)}
                </div>
            )}
          </nav>
        </aside>
        ...
        <>
            {
                ...
                ? 
                <Link to="/">
                <h2 onClick={() => setActive('Home')} className="text-sm md:text-lg font-semibold text-zinc-500 dark:text-zinc-400" >
                    {group.name}
                </h2>
                </Link>
                :
