...
const COLORS = ['rgb(220,168,154,1)', 'rgb(209,138,154,0.8)', 'rgb(209,138,154,0.5)', 'rgb(209,138,154,0.2)'];
...
const yValueFormatter = (yValue) => kFormatter(yValue).toLocaleString('en-US', { style: 'currency', currency: 'USD' })
      ...
      <PieChart >
        ...
        >
          {data.map((entry, index) => (
            <Cell stroke="rgb(209,138,154,0.2)" key={`cell-${index}`} fill={entry.name === trafficSource ? COLORS[0] : COLORS[COLORS.length - 1]}/> //COLORS[index % COLORS.length]} />
          ))}
          <Label
  ...
  {type === 'area' ?
      <AreaChart data={data}>
        ...
        <Tooltip
                  ...
                  />
        <Area dataKey={getYValue} stroke="#d18" fill="url(#color)" radius={[4, 4, 0, 0]} animationDuration={1000} animationEasing="linear" />
      </AreaChart>
    :
    <BarChart data={data}>
      <XAxis
        ...
        <Tooltip
                  ...
                  />
        <Bar dataKey={getYValue} stroke="#d18" fill="url(#color)" radius={[4, 4, 0, 0]} animationDuration={1000} animationEasing="linear" />
    </BarChart>
    }
  </VizContainer>
))
        ...
        <div className="grid gap-y-4 gap-x-0 xl:gap-4 grid-cols-1 lg:grid-cols-1 xl:grid-cols-6">
            <div className="col-span-5 rounded-xl light:border bg-card text-card-foreground bg-white dark:bg-zinc-900 shadow hover:brightness-100 hover:drop-shadow-lg">
              <div className="flex flex-row justify-between align-middle items-center">
                <div className="flex flex-col space-y-1.5 p-6">
                  <h3 className="font-semibold leading-none tracking-tight text-black dark:text-white">Overview</h3>
                </div>
                <div className="flex flex-row space-x-2 pr-2">
                  <button onClick={() => setViz('line')} className="text-black dark:text-white inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2">
                    Bar
                  </button>
                  <button onClick={() => setViz('area')} className="text-black dark:text-white inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2">
                    Area
                  </button>
                </div>
              </div>
            ...
            <div className="rounded-xl light:border bg-card text-card-foreground bg-white dark:bg-zinc-900 shadow hover:brightness-100 hover:drop-shadow-lg">
              <div className="flex flex-col space-y-1 p-4">
                <h3 className="font-semibold leading-none tracking-tight text-black dark:text-white">Sign Ups by Source</h3>
              </div>
              <div className="p-4 pt-0 pl-2 grid gap-2 grid-cols-2">
                {['Search', 'Google Ads', 'Facebook', 'Bing'].map(val => (
                  <button onClick={() => setTrafficSource(val)} className="text-black dark:text-white inline-flex items-center justify-center rounded-md text-sm font-medium transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-50 border border-input bg-transparent shadow-sm hover:bg-accent hover:text-accent-foreground h-9 px-4 py-2">
                    {val}
                  </button>
                ))}
              </div>
            ...
            <div className="rounded-xl light:border bg-card text-card-foreground bg-white dark:bg-zinc-900 shadow hover:brightness-100 hover:drop-shadow-lg">
              ...
              <div className="p-6 pt-0 pl-2">
                {trafficSource && <DashboardPieChart trafficSource={trafficSource}/>}
              </div>
            </div>
            </div>
        </div>
